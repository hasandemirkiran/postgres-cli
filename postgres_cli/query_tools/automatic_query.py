...
def fetch_from_label_db(query):
    ...
    try:
        params = db_config(filename='database.ini', section='postgresql')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--query_type", choices=['similar_conditions', 'temporal_intrval', 'single_tree_percentage', 'region_name'], type=str, help="Type of query used for the label database.")
    
    # single_tree_percentage parameters
    parser.add_argument("--reference_tree", type=str, help="Class of tree to use as reference one")
    parser.add_argument("--max_percentage", type=int, default=100, help="Maximum percentage of the specific tree present in the bbox")
    parser.add_argument("--min_percentage", type=int, default=0, help="Minimum percentage of the specific tree present in the bbox")
    ...
    parser.add_argument("--days", default=10 ,type=int, help="Interval of days to analyze before and after the starting date. Input format YYYY-MM-DD")
    parser.add_argument("--months", default=0, type=int, help="Interval of months to analyze before and after the starting date. Input format YYYY-MM-DD")
    ...
    try:
        # Fetch data
        df = fetch_from_label_db('SELECT id AS id, name AS name FROM public.tree_class;')
    ...
    out_folder = os.path.join(args.output_path, 'training_data_{}/'.format(current_time))
    os.mkdir(out_folder)
    ...
    try:
        if args.query_type == 'single_tree_percentage':
            ...
            # Configure the query
            query = queries_file.return_all_region_by_tree_type_percentage(tree_types[args.reference_tree], args.min_percentage, args.max_percentage)
            ...
            for index, row in data.iterrows():
                ...
                if actual_region == None:
                    ...
                    if actual_region != name:
                        ...
                        update_config(config, os.path.join(out_folder, actual_region), actual_raster)
                ...
                coordinates= [tmp['coordinates'][0][0][0], tmp['coordinates'][0][0][1], tmp['coordinates'][0][2][0],tmp['coordinates'][0][2][1]]
                features.append(Feature(geometry=Point((row['feature_coordinate_x'],row['feature_coordinate_y']), precision=14), properties={"class": row['tree_type'], 
                                        "bbox_id": row["label_id"]}, bbox=[coordinates[0],coordinates[2],coordinates[3],coordinates[1]])) # Be aware, the bbox id is not the same of the original label file
            ...
            update_config(config, os.path.join(out_folder, args.region), row['raster_url'])
            print('--created {} with {} points'.format(args.region, len(features)))  
            print(row['raster_url'])


        elif args.query_type == 'similar_season':
