...
def find_orthoURL_of_bbox(bbox,ortho_url_bounds_dict):
    for url in ortho_url_bounds_dict:
        ...
        if bbox[2]<=ortho_bounds[2] and bbox[2]>=ortho_bounds[0] and  bbox[3]<=ortho_bounds[3] and bbox[3]>=ortho_bounds[1]:
            return url

def split_label(label_URL):
    label_bbox_ortho_dict = {}
    ortho_url_bounds_dict_url = r'c:\Users\hasan\OneDrive\Masaüstü\postgres-cli\Utils\ortho_url_bounds_dict.txt'
...
def create_splitted_files(label_bbox_ortho_dict, label_name):
    ...
    for url in label_bbox_ortho_dict:
        ...
        with open( label_name + '__ortho__' + ortho_name + '__splitted' + '__Hasan.json', 'w') as convert_file:
            convert_file.write(json.dumps(label_bbox_ortho_dict[url]))
     

if __name__ == "__main__":
    label_URL = '\\\\192.168.37.4\\ml\\datasets\\forestry\\Toering\\labels_v1_plus_v2.json'
